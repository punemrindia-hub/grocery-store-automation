...
const posInput = z.object({ customerId: z.string().min(1).optional(), customerName: z.string().trim().min(1).max(120).optional(), customerWhatsapp: z.string().trim().min(8).max(20).optional(), paymentMethod: z.nativeEnum(PaymentMethod).default(PaymentMethod.CASH), fulfillmentType: z.nativeEnum(FulfillmentType).default(FulfillmentType.TAKEAWAY), items: z.array(itemInput).min(1) });
...
export async function registerOrderRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.post('/api/v1/pos/orders', async (request, reply) => {
    ...
    const order = await prisma.$transaction(async (transaction) => {
      ...
      const payment = await transaction.payment.create({ data: { shopId, customerId: customer!.id, orderId: created.id, amount: pricing.total, method: input.paymentMethod, status: paymentStatus, provider: 'POS' } });
      if (input.paymentMethod !== PaymentMethod.CREDIT) await transaction.ledgerEntry.create({ data: { shopId, customerId: customer!.id, orderId: created.id, paymentId: payment.id, type: LedgerType.PAYMENT, direction: LedgerDirection.CREDIT, amount: pricing.total, description: `POS ${input.paymentMethod.toLowerCase()} payment`, reference: payment.id, createdBy: userId } });
