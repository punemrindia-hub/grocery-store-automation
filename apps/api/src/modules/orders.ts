...
const posInput = z.object({ customerId: z.string().min(1).optional(), customerName: z.string().trim().min(1).max(120).optional(), customerWhatsapp: z.string().trim().min(8).max(20).optional(), paymentMethod: z.nativeEnum(PaymentMethod).default(PaymentMethod.CASH), fulfillmentType: z.nativeEnum(FulfillmentType).default(FulfillmentType.TAKEAWAY), items: z.array(itemInput).default([]), miscAmount: z.number().nonnegative().default(0), miscDescription: z.string().trim().max(100).default('Miscellaneous charge') }).refine((data) => data.items.length > 0 || data.miscAmount > 0, { message: 'Add at least one product or a custom amount' });
